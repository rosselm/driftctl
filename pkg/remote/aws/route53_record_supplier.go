...
func (s Route53RecordSupplier) Resources() ([]resource.Resource, error) {
	...
	if err != nil {
		return nil, NewBaseListError(err, resourceaws.AwsRoute53RecordResourceType, resourceaws.AwsRoute53ZoneResourceType)
	}

	for _, zone := range zones {
		if err := s.listRecordsForZone(zone[0], zone[1]); err != nil {
			return nil, NewBaseListError(err, resourceaws.AwsRoute53RecordResourceType, resourceaws.AwsRoute53RecordResourceType)
		}
	}
